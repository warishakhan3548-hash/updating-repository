...
`tools/pack_gate.py` fails closed when the source is not production-approved, when source identity/hash/path/licence drift from the Source Vault registry, or when the built artifact's bytes no longer match its manifest. For Quran core releases, byte identity is followed by an importer-independent semantic check against the preserved Source Vault rows; recomputing a manifest hash after changing sacred content does not make the pack valid.

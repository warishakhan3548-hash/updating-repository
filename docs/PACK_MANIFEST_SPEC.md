...
- `approved`: semantic/provenance gates passed and cryptographic signature threshold verified.

For `approved`, `tools/pack_gate.py` first completes the applicable source, licence, artifact and semantic-fidelity checks, then delegates to `tools/pack_signing.py`. The manifest must carry a positive signed `release_sequence` plus Ed25519 signatures using payload format `aaris-pack-json-v1`; enough non-revoked keys from `policy/trusted_pack_keys.json` must verify to meet `signature_threshold`.

The production trusted-key policy is intentionally empty today, so no current pack is approvable yet. Signature-shaped strings alone never pass. See `PACK_SIGNING.md` for exact payload, key rotation and rollback-metadata rules.

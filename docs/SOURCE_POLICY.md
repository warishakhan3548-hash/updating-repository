...
**Archival retention is a universal preservation gate.** Any source entering `awaiting-artifact`, any source with preserved bytes, and every `production-approved` source must have an explicit `historical_snapshot_retention_status=verified-allowed`. General redistribution permission alone is not enough to prove that an immutable historical snapshot may remain in the project-controlled archive.

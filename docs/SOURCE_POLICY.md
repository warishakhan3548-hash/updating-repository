...
Historical mirror permission is a separate production gate. If the inspected licence does not clearly establish that superseded source versions may remain in the project's long-lived immutable redistribution archive, set `historical_snapshot_retention_status` to `unresolved` (or `verified-not-allowed`). Such a source cannot be `production-approved`; a current-version release review cannot override that blocker. Promotion requires explicit permission or a preservation/distribution design whose legality has been independently verified.

...
| Hadith research | Local-only Hadith UI, separate checksum-verified read-only `HadithStore`, deterministic JSONL→SQLite builder, collection/book/chapter navigation, local Arabic/English/reference search, edition-aware IDs, alternate references and attributed grades; no approved corpus is installed yet | Populate the full catalog with redistribution-cleared source records, validate edition/numbering coverage, add narration clusters and a labeled retrieval evaluation |

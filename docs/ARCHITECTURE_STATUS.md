...
| Hadith research | Local-only Hadith UI, separate checksum-verified read-only `HadithStore`, deterministic source→SQLite builders, collection/book/chapter navigation, local Arabic/English/reference search, edition-aware IDs, alternate references and attributed grades; checked-in hash-locked Open-Hadith-Data provides a real Arabic core-nine fallback (62,169 records) | Extend beyond the core nine with redistribution-cleared source records, validate cross-edition numbering/translation coverage, add narration clusters and a labeled retrieval evaluation |
...
| Evaluation | Dependency-free JVM regressions; whole-corpus coordinate checks; small positive/absent query regression set; network-free synthetic Quran-audio pack/dedup/corruption self-test; local Hadith reconstruction/integrity checks | Scholar/reviewer labels, larger multilingual/zero-answer evaluation, false-positive calibration and phone latency/battery profiling |
...
- Full Gradle execution could not resolve Android Gradle Plugin 8.9.2 in this environment.
  Releases 0.2.0 and 0.3.0 were subsequently built with the official SDK tools using `tools/build_release.py`.
  Its non-debuggable APK, v2/v3 signature, alignment, DEX checksums and embedded source pack were
  checked. See `docs/RELEASE_0.3.0.md` and `docs/AMBIENT_RECALL.md`. No emulator launch, actual phone test or pixel-perfect

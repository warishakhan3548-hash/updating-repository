...
- deterministic `rare-word-rescue-v1` Learning Plane policy that enrolls only from personal struggle/scheduler evidence, keeps unknown retrievability unknown, defers an already-due review to a natural encounter only with explicit scheduler authorization, and never treats passive reading as successful recall; it remains dormant for Quran words until provenance-backed canonical word identities exist;
...
- executable host-side `aaris-evidence-bundle-v1` for explicit Quran ayah evidence: deterministic JSON + UTF-8 text + checksums, existing pack-gate delegation, approved-by-default production posture, explicit candidate-only development override, source-faithful Arabic plus the exact validated source attribution notice, exact local record reconstruction, mandatory export-hash pinning and returned-citation scope enforcement; the module has no network/provider dependency and reports only **References verified**, never conclusion verification;
...
- approved pack releases now require signed canonical UTC issue/expiry metadata with a 366-day maximum window; structural verification remains timeless for reproducibility while the separate activation policy rejects not-yet-valid/expired metadata for a new trust decision, and explicit recovery rules retain the current verified pack instead of weakening freshness/rollback checks;
...
- Extended Quranic Treebank (EQTB) v1: `awaiting-licence`. Mendeley labels the complete public dataset CC BY 4.0, but its data paper traces foundational morphology/syntax to Quranic Corpus material and an institutional archive explicitly preserves third-party component licences. Until that component rights chain is shown compatible with commercial immutable archival redistribution, no EQTB bytes are mirrored or consumed.
...
Reader Core regression coverage checks read-only SQLite access, fail-closed coordinates, original-text-only models, navigation edges, complete 6,236-coordinate iteration, and the invariant that the current ayah-only pack still contains zero canonical `quran_token` rows. Android JVM coverage also locks representative `arabic-search-v1` query-normalization vectors while the search UI remains fail-closed on normalization-version mismatch. Signing regression coverage verifies valid Ed25519 approval, post-signing tamper failure, unauthorized/duplicate keys, signed release ordering, retired-key historical windows, revoked-key rejection, active-threshold viability, malformed/bootstrap trust roots, strict JSON/domain separation, Android release delegation to the authoritative pack gate, and the approved-release freshness-window contract including expiry/not-yet-valid activation rejection.

Evidence-export regression coverage now locks production rejection of the unsigned candidate, deterministic bundle bytes/hashes, source-faithful record projection, create-only default output behavior, exact-local tamper rejection, export-hash pinning and rejection of citations outside the exported scope. Quran search now has an executable host-side golden benchmark. Its host SQLite latency is diagnostic only and is not presented as low-end Android performance. Android JVM unit coverage now also exercises the deterministic rare-word rescue policy, including fail-closed scheduler-owned substitution, enrollment thresholds and priority-signal math. JVM coverage also locks the context-rotation policy's binding verification, familiar-to-varied progression, Hadith opt-in and deterministic tie-breaking. No Hadith retrieval benchmark, FSRS retention benchmark, rare-word retention benefit, accessibility device test or low-end Android performance number is claimed yet because those systems are not mature enough to measure honestly. The learning ledger and rescue policy establish durable/testable behavior only; they do not claim measured retention improvement.
...
2. Signed static freshness/expiry metadata and explicit recovery behavior are now implemented. Before enabling any automatic remote content-update channel, add a short-lived authenticated online freshness layer, downloaded-pack staging/activation tests, a reviewed API-24-compatible on-device signature verifier, and remote trust rotation/revocation. Android platform Ed25519 alone is insufficient because it is available only from API 33 while this app supports API 24+.
...
5. Obtain QuranMorph through an authorized publisher path and verify its exact artifact, licence snapshot, checksum, and 6,236-ayah coordinate alignment; keep QAC/QUL morphology and QUL English/Hindi WBW data blocked unless their own resource-specific gates clear. Only after canonical word/occurrence identity exists should verified occurrence predictions activate `rare-word-rescue-v1`. Resolve HadeethEnc historical-snapshot retention and commercial-use rights (or select another legally durable edition-aware Hadith source) before preserving Hadith bytes or building production Hadith search.
6. Add an independent backup/archive for critical Source Vault artifacts and trust-root history.
7. After the host evidence-bundle contract is stable, add the simple Android share flow and a tested Arabic/RTL-accessible PDF renderer over the same bundle. Extend the bundle to Hadith only after an edition-aware Hadith source clears the Source Vault and production-pack gates.

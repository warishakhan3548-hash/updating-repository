...
- deterministic `rare-word-rescue-v1` Learning Plane policy that enrolls only from personal struggle/scheduler evidence, keeps unknown retrievability unknown, can defer an interruption to a suitable imminent natural encounter, and never treats passive reading as successful recall; it remains dormant for Quran words until provenance-backed canonical word identities exist;

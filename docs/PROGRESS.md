...
- versioned `quran-search-golden-v1` evaluation set plus deterministic host evaluator for Recall@5/10, MRR, NDCG@10, negative false-positive rate and zero-result rate; CI protects exact/no-harakat/partial retrieval plus the constrained orthographic/keyboard lanes while keeping unsupported typo behavior measured rather than claimed;
...
3. Complete accessibility/device validation for the minimal Android reader. Keep word tap hidden until a provenance-backed linguistic/gloss pack is installed, then expose it with an equivalent accessible action rather than a gesture-only path.

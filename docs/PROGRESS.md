...
- the single `tools/capture_quranenc_gloss.py` owner still refuses new network acquisition unless the registry is explicitly `awaiting-artifact` with reviewed permissions, and now also validates the already-preserved review-only QuranEnc layout fully offline: official index/terms/source page, exactly 114 Surah API responses / 6,236 coordinates, exact requested URLs, per-file byte sizes and SHA-256, canonical aggregate manifest, provenance and `sha256.txt`.
...
- QuranEnc Arabic Meanings of Words (As-Siraj) v1.0.0: `awaiting-licence`; an exact v1.0.0 review-only snapshot is already preserved under project control and offline-verifiable, but its published republication conditions include staying updated to newer source versions and immutable public redistribution of superseded historical snapshots is not yet clearly authorized. The preserved bytes are not a production dependency, and no additional/replacement capture is allowed while that question remains unresolved.
...
4. Resolve QuranEnc’s archival-redistribution ambiguity first: its published republication terms require updates to newer source versions, while this project requires immutable historical snapshots. Keep the already-preserved v1.0.0 candidate non-production and `awaiting-licence`; if a durable legal basis is established, independently revalidate the preserved snapshot, add the reviewed multi-file Source Vault contract, perform a fresh upstream-version review, and only then consider production promotion or any authorized replacement capture.

...
- immutable historical `quran-core 1.0.4` schema-v2 candidate retained for reproducibility;
- current `quran-core 1.1.0` schema-v3 candidate published with canonical JSONL binding and importer/toolchain metadata;
...
Candidate does not mean release-approved. Version 1.1.0 adds a deterministic canonical semantic anchor without modifying the preserved Tanzil Quran source. The Android debug reader now exercises this candidate; release builds still require `approved` plus a valid project-trusted Ed25519 signature.

## Validation status

Automated coverage now checks Source Vault integrity, licence/provenance consistency, schema-v2 manifest-to-vault binding, manifest-to-SQLite provenance/notice consistency, required attribution-notice hashing, pack-local artifact/notice isolation (including symlink resolution), SQLite schemas, sacred-text immutability, append-only learning events, Quran coordinate ordering, source hash/size, direct builder CLI execution, and deterministic pack reproducibility. The protected publisher generated and committed canonical Quran 1.0.0 plus quran-core 1.1.0 only after exact-tree Source Vault, pack, schema and unit-test validation. CI now also rejects movable remote Action references, pins external Actions to verified full commit SHAs, and requires future generated-pack commits to revalidate Source Vault, pack, schema and unit-test gates on the exact committed tree before push.
...
Manifest schema v3 and `canonical/quran-core/1.0.0/ayahs.jsonl` are now published on main together with the immutable `quran-core 1.1.0` candidate. Schema-v2 verification remains for historical 1.0.4. Neither candidate is production-approved.

...
- the exact QuranEnc v1.0.0 review candidate is already preserved at `source-vault/quran-gloss/quranenc/arabic-seraj/1.0.0/`: 114 Surah API responses / 6,236 coordinates plus source/index pages, terms, provenance and checksums. Snapshot-manifest SHA-256: `8cbc4f5f41298e438f7862fff1eba309ffdab254ca240257bd5b652631f2396c`; terms SHA-256: `24dd28bc25f21e59a2ec87137faeb0d969c1401232d99d2177cabf887c48341c`. It remains captured-unreviewed and unbound from production; the acquisition gate now refuses new network capture while the registry is `awaiting-licence`.
...
- QuranEnc Arabic Meanings of Words (As-Siraj) v1.0.0: `awaiting-licence`; exact review-only bytes are already preserved, but they are not a production Source Vault binding. Immutable public redistribution of superseded historical snapshots is not yet clearly authorized, so promotion and future durable-version capture remain blocked.
...
4. Resolve QuranEnc’s archival-redistribution ambiguity before production use. The v1.0.0 review-only snapshot is already captured; keep it unbound from production and block future durable captures while `awaiting-licence`. If written clarification or another durable legal basis permits historical retention, validate and bind the exact preserved candidate through the multi-file Source Vault gate, then separately review any production promotion.

...
- executable host-side `aaris-evidence-bundle-v1` for explicit Quran ayah evidence: deterministic JSON + UTF-8 text + checksums, existing pack-gate delegation, approved-by-default production posture, explicit candidate-only development override, source-faithful Arabic only, exact local record reconstruction, optional export-hash pinning and returned-citation scope enforcement; the module has no network/provider dependency and reports only **References verified**, never conclusion verification;

...
- one-shot `tools/capture_quranenc_gloss.py` acquisition gate pins QuranEnc `arabic_seraj` v1.0.0, validates the complete 6,236-coordinate shape, rejects source drift/off-host redirects/partial capture/overwrite, and emits deterministic review-only metadata. A prior review capture exists in Git history at `95e8fc41fae70c4687e41ac887df7ab0c2c2eb48`, but the unresolved candidate bytes are removed from the active Source Vault tree. New acquisition now requires `awaiting-artifact`, reviewed permissions, and verified historical snapshot retention before any network fetch.
...
4. Resolve QuranEnc’s archival-redistribution ambiguity first: its published republication terms require updates to newer source versions, while this project requires immutable historical snapshots. Keep it `awaiting-licence`; only after written clarification or another durable legal basis verifies historical retention may it move to `awaiting-artifact`. At that point restore/reacquire a legally permitted exact snapshot through the gated path, validate it, and separately perform the latest-version release review before any production promotion.

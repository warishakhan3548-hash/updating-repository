...
- preserved `quran-search-golden-v1` baseline plus active `quran-search-golden-v2` evaluation set and deterministic host evaluator for Recall@5/10, MRR, NDCG@10, negative false-positive rate and zero-result rate; CI now protects exact/no-harakat/partial plus conservative orthographic/keyboard compatibility recall without pretending typo/fuzzy retrieval is already solved;

...
- QuranEnc `arabic_seraj` v1.0.0 is now preserved under project control as a review-only multi-file Source Vault snapshot: official source/index/terms pages plus exact 114 Surah API response bodies covering all 6,236 ayahs, with per-file SHA-256, immutable capture provenance and aggregate manifest `8cbc4f5f41298e438f7862fff1eba309ffdab254ca240257bd5b652631f2396c`; it remains blocked from production because historical-retention permission is unresolved.
- `tools/capture_quranenc_gloss.py` is the single QuranEnc acquisition/verification owner. It validates exact canonical request URLs, trusted-host redirects, bounded retries/response sizes, exact per-Surah coordinate sets, mid-capture source-index stability, immutable destinations and full offline reconstruction of the preserved member/checksum set. Normal CI performs offline validation only; it does not contact upstream.
...
- QuranEnc Arabic Meanings of Words (As-Siraj) v1.0.0: `research-candidate / captured-unreviewed`; exact content and terms are preserved and integrity-gated, but production use remains blocked until permanent historical-retention permission is verified and a release-time latest-version review can be satisfied.
...
4. Resolve QuranEnc's historical-retention permission explicitly. If cleared, perform a separate reviewed promotion using the already-preserved v1.0.0 bytes plus a fresh upstream-version check, then prototype a verse-scoped gloss pack; do not infer morphology or lexical IDs from it or from AI. If permission is not cleared, keep the snapshot non-production and replace it with a legally durable source.

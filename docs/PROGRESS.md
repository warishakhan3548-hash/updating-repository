...
- primary-source audit identified QuranEnc `arabic_seraj` v1.0.0 as a promising verse-scoped difficult-word gloss source and defined a fail-closed gloss bridge that does not fabricate morphology or lexical IDs; source is `awaiting-licence` pending clarification that immutable historical archival redistribution remains permitted after newer upstream versions appear.
- canonical `tools/capture_quranenc_gloss.py` now fails closed before network access unless registry permissions and immutable historical-retention rights are explicitly cleared, then pins QuranEnc `arabic_seraj` v1.0.0 from the official source index, preserves exact pre/post index + source/terms pages + 114 Surah responses, validates all 6,236 coordinates, and emits deterministic review-only snapshot metadata.
...
- QuranEnc Arabic Meanings of Words (As-Siraj) v1.0.0: exact review bytes for 114 Surahs / 6,236 ayahs are preserved under `source-vault/quran-gloss/quranenc/arabic-seraj/1.0.0/`, but the registry remains `awaiting-licence`; the current terms do not clearly establish permanent public redistribution of superseded immutable snapshots, so future capture and all production promotion remain blocked.
...
4. Resolve QuranEnc’s archival-redistribution ambiguity. Keep the preserved v1.0.0 review snapshot outside production and the registry `awaiting-licence`; after written clarification or another durable legal basis verifies historical retention, bind the existing snapshot through the reviewed multi-file Source Vault gate (or capture a newer upstream version if required), then separately review any runtime-pack promotion.

...
- Ed25519 release-authenticity gate with deterministic signed-manifest bytes, content-derived key IDs, threshold policy, unauthorized/duplicate-signature rejection, and project-controlled public trust-root storage;
- hardened signing representation with domain separation, duplicate-JSON-field rejection, ASCII object keys, safe-range integers, no floats, and signed monotonic `release_sequence`;
- Android release builds delegate to the authoritative pack gate instead of trusting signature-shaped metadata;
...
Candidate does not mean release-approved. Version 1.1.0 adds a deterministic canonical semantic anchor while retaining independent Source Vault/runtime semantic verification. The Android debug reader now bundles this candidate so the newest trust path is exercised without enabling production approval.

## Validation status

Automated coverage now checks Source Vault integrity, licence/provenance consistency, schema-v2 manifest-to-vault binding, manifest-to-SQLite provenance/notice consistency, required attribution-notice hashing, pack-local artifact/notice isolation (including symlink resolution), SQLite schemas, sacred-text immutability, append-only learning events, Quran coordinate ordering, source hash/size, direct builder CLI execution, and deterministic pack reproducibility. The historical 1.0.4 publisher passed its then-current 44-test suite before publishing. The newer 1.1.0 canonical/runtime artifacts were generated by the protected publisher from the pinned Source Vault. CI now also enforces full-SHA Action pins, an exactly pinned release-verifier dependency, explicit trust-root validation, and exact-tree Source Vault/pack/schema/unit-test revalidation before a generated commit may be pushed.
...
Canonical v3 is now published as an immutable candidate: `canonical/quran-core/1.0.0/ayahs.jsonl` anchors `quran-core 1.1.0`. Historical schema-v1/v2 candidates remain unchanged and reproducible under their original contracts.

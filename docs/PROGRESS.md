...
5. Obtain QuranMorph through an authorized publisher path and verify its exact artifact, licence snapshot, checksum, and 6,236-ayah coordinate alignment; keep QAC/QUL morphology and QUL English/Hindi WBW data blocked unless their own resource-specific gates clear. Resolve HadeethEnc historical-snapshot retention and commercial-use rights (or select another legally durable edition-aware Hadith source) before preserving Hadith bytes or building production Hadith search.

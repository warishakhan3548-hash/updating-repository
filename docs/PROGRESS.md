...
- generic checksum-set verification for immutable multi-file source snapshots, including traversal/symlink/alias/tamper rejection;
- preserved QuranEnc `arabic_seraj` v1.0.0 review snapshot verified offline across its exact 119 checksum-bound files, 114 Surahs and 6,236 coordinates; it remains non-production pending archival-rights resolution;
...
- primary-source audit identified QuranEnc `arabic_seraj` v1.0.0 as a promising verse-scoped difficult-word gloss source and defined a fail-closed gloss bridge that does not fabricate morphology or lexical IDs.
- the exact QuranEnc review snapshot is preserved at `source-vault/quran-gloss/quranenc/arabic-seraj/1.0.0/`; `tools/verify_quranenc_snapshot.py` now revalidates it without network access, and the superseded one-shot downloader/tests have been removed so there is one owner for the preserved-snapshot responsibility.
...
- QuranEnc Arabic Meanings of Words (As-Siraj) v1.0.0: `awaiting-licence`; exact current-version content, source/index page, applicable terms, provenance and all 114 API response files are preserved and checksum-verified under project control. Production remains blocked because the inspected terms do not clearly establish indefinite public retention of superseded editions.
...
4. Resolve QuranEnc historical archival-retention rights through explicit permission or a legally compatible preservation design. Only after that gate clears should the captured v1.0.0 snapshot be registered for production promotion and used to prototype a verse-scoped gloss pack; do not infer morphology or lexical IDs from it or from AI.

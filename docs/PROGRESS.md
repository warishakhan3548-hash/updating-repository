...
- primary-source audit identified QuranEnc `arabic_seraj` v1.0.0 as a promising verse-scoped difficult-word gloss source and defined a fail-closed gloss bridge that does not fabricate morphology or lexical IDs; source is `awaiting-licence` while historical public-mirror retention remains unresolved.
- one-shot `tools/capture_quranenc_gloss.py` acquisition gate pins QuranEnc `arabic_seraj` v1.0.0, preserves exact pre/post metadata + 114 Surah response byte streams + official terms/source page, validates the complete 6,236-coordinate shape, rejects source drift/off-host redirects/partial capture/overwrite, and emits deterministic review-only snapshot metadata. It now refuses network acquisition until archival retention is verified and the registry is explicitly capture-authorized; it is not a normal-build dependency and does not promote the registry.
...
- QuranEnc Arabic Meanings of Words (As-Siraj) v1.0.0: `awaiting-licence`; current republication terms require staying current, while indefinite public retention of superseded snapshots is not clearly established. Acquisition is blocked until archival retention is verified; a later approved pack would still require the separate signed latest-version review.
...
4. Resolve QuranEnc historical snapshot retention first. Only after archival permission is verified may the registry move to `awaiting-artifact` and the reviewed one-shot capture run; release-time latest-version attestation remains a separate later gate. Do not infer morphology or lexical IDs from this source or from AI.

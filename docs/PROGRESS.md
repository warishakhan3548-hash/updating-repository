...
- dormant Android `rare-word-rescue-v1` domain policy over trusted semantic units: personal-struggle hard gate, scheduler-neutral forgetting-risk input, exposure-scarcity × personal-relevance prioritization, natural-reading substitution for imminent due encounters, and deterministic context rotation from familiar to varied verified contexts; no word identity, Evidence Plane data, or visible reader UI is fabricated;
...
Quran search now has an executable host-side golden benchmark. Its host SQLite latency is diagnostic only and is not presented as low-end Android performance. No Hadith retrieval benchmark, FSRS retention benchmark, accessibility device test or low-end Android performance number is claimed yet because those systems are not mature enough to measure honestly. The learning-ledger change establishes durable inputs only; it does not claim that FSRS retention quality has been measured. JVM regression coverage now also locks the scheduler-neutral rare-word rescue policy: rarity without personal struggle cannot schedule intervention, imminent natural reading may substitute for a due duplicate drill without counting passive exposure as recall, and context rotation rejects unverified sources. This remains policy validation, not a retention-efficacy claim.
...
5. Obtain QuranMorph through an authorized publisher path and verify its exact artifact, licence snapshot, checksum, and 6,236-ayah coordinate alignment; keep QAC/QUL blocked unless their own gates clear. Resolve HadeethEnc historical-snapshot retention and commercial-use rights (or select another legally durable edition-aware Hadith source) before preserving Hadith bytes or building production Hadith search.
   Once a trusted semantic-unit source is production-approved, wire `rare-word-rescue-v1` to real exposure/review events and evaluate it against a labelled learning protocol before claiming retention gains or exposing automatic review UI.

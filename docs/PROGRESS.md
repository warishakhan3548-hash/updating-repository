...
- primary-source audit identified QuranEnc `arabic_seraj` v1.0.0 as a promising verse-scoped difficult-word gloss source and defined a fail-closed gloss bridge that does not fabricate morphology or lexical IDs; the exact review-only source snapshot is now preserved, while production remains `awaiting-licence` because permanent historical-retention permission is unresolved.
- the single `tools/capture_quranenc_gloss.py` owner now uses the proven official source-index + 114-Surah API layout, preserves exact source/index/terms bytes, and can revalidate the merged review-only snapshot fully offline against per-file SHA-256, exact 114-Surah / 6,236-coordinate invariants, canonical snapshot metadata, provenance and `sha256.txt`; it remains outside normal builds and does not promote the source.
...
- QuranEnc Arabic Meanings of Words (As-Siraj) v1.0.0: `awaiting-licence`; exact source/index/terms bytes and all 114 Surah API responses are preserved under project control and remain `captured-unreviewed`. Production is blocked because permanent retention of superseded public snapshots is not yet established under the source's latest-version/update obligation.
...
4. Complete explicit QuranEnc legal/durability review for permanent historical snapshot retention, run the offline validator over the already-preserved v1.0.0 capture, and promote only in a separate reviewed registry change if that permission is established; only then prototype the verse-scoped gloss pack, without inferring morphology or lexical IDs from it or from AI.

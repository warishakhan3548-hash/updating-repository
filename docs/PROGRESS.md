...
- Android production activation persists the highest accepted signed `release_sequence` with `AtomicFile` under `noBackupFilesDir`, enforces the signed-metadata safe-integer range, rejects lower bundled releases before activation, and leaves debug candidates outside that trust state;

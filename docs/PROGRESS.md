...
5. Obtain QuranMorph through an authorized publisher path and verify its exact artifact, licence snapshot, checksum, and 6,236-ayah coordinate alignment; keep QAC/QUL blocked unless their own gates clear. Only after canonical word/occurrence identity exists should verified occurrence predictions activate `rare-word-rescue-v1`. Resolve HadeethEnc historical-snapshot retention and commercial-use rights (or select another legally durable edition-aware Hadith source) before preserving Hadith bytes or building production Hadith search.

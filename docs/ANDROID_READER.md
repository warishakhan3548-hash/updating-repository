...
The state and installed pack replacement use Android `AtomicFile` so interrupted replacement falls back to the prior complete file rather than leaving a partial trust record or content database. Because `AtomicFile` does not provide locking, one process-wide monitor serializes the complete release preflight → pack activation → acceptance-state commit transaction across repository instances. The current app is single-process; adding another Android process would require an explicit cross-process coordination design. This is an installed-app rollback barrier, not hardware-backed monotonic storage: uninstall or clear-data removes the state. Automatic downloaded content updates remain disabled.
...
- changing this trust contract requires a new immutable content version rather than rewriting an older pack;\n- runtime SQLite schema must match the reviewed canonical schema, and ayah-only Quran core must contain no undeclared morphology/Hadith evidence.\n\nA runtime SHA-256 identifies exact shipped bytes. Schema-v2 promotion additionally proves semantic fidelity to Source Vault evidence; recomputing a hash after altering sacred text or the canonical schema must not make a pack valid.

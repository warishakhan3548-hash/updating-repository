...
| HadeethEnc Arabic | Hadith evidence candidate | **awaiting-licence** | Official Arabic v1.7.0 republication terms include an obligation to update according to newer source versions. That does not yet clearly establish indefinite public retention of superseded snapshots, so no bytes may enter the immutable Source Vault until archival retention is clarified. Edition/collection/numbering provenance remains a separate later gate. |

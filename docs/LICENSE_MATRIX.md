...
| QuranEnc Arabic Meanings of Words (As-Siraj) v1.0.0 | contextual difficult-word gloss candidate | **awaiting-licence** | Official catalogue identifies v1.0.0 and permits republication under source-specific conditions, including updating to the latest issued version. An exact v1.0.0 review snapshot was captured while that version was current, but immutable public redistribution of superseded snapshots is not yet clearly authorized. The registry therefore remains `awaiting-licence`, future capture/promotion is blocked, and no runtime pack may consume it. If archival permission is verified, release approval must still bind a signed source-release review to the official source index. It is a gloss source, not a morphology or Quran-text authority. |

...
| QuranEnc Arabic Meanings of Words (As-Siraj) v1.0.0 | contextual difficult-word gloss candidate | **awaiting-licence** | Official catalogue identifies v1.0.0 and QuranEnc permits republication under source-specific conditions, including updating to the latest issued version. The inspected terms do not clearly establish indefinite public retention of superseded versions, so acquisition into the immutable public Source Vault is blocked while `historical_snapshot_retention_status` is unresolved. If archival permission is later verified, release approval still separately requires the signed latest-version review. It is a gloss source, not a morphology or Quran-text authority. |

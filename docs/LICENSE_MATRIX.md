...
|---|---|---|---|
| Tanzil Quran Text v1.1 | Quran Evidence Plane source | **production-approved** | Exact Uthmani `txt-2` snapshot is preserved in Source Vault. Official terms allow verbatim copying/distribution with attribution/source link and prohibit changing the Quran text; historical retention is explicitly `verified-allowed`, while no mandatory latest-version release review is imposed. |
...
| QuranMorph (SinaLab/Birzeit, 2025) | morphology candidate | **awaiting-artifact** | Official SinaLab catalogue labels the Quran morphology dataset CC BY 4.0, whose standard grant supports durable reproduction/sharing, so historical retention is `verified-allowed`. The current publisher download path is still access-gated; no exact bytes/version have been acquired or mirrored, and the paper's 6,235-verse count still requires reconciliation against the 6,236-ayah Tanzil coordinates before promotion. |
...
Historical-retention permission is now an independent machine-readable gate. Every `awaiting-artifact`, preserved, or `production-approved` source must record `verified-allowed`; a separate boolean records whether approved releases must prove they still match the latest upstream version.

...
| HadeethEnc Arabic | Hadith evidence/search candidate | **awaiting-licence** | Official version check currently reports Arabic v1.7.0 and the site permits download/republication under source-specific conditions, but it also requires updating according to newer source releases. Those terms do not yet clearly establish indefinite public redistribution of superseded snapshots, so immutable Source Vault capture is blocked. Even if archival retention is later cleared, exact edition/collection/numbering provenance remains a separate Evidence Plane gate. |

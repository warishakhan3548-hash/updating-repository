...
| HadeethEnc Arabic | Hadith source candidate | **awaiting-licence** | Official version check reports Arabic v1.7.0. Current republication terms permit reuse only under source-specific conditions and require downstream copies to track newer versions. Because those terms do not clearly establish indefinite public redistribution of superseded snapshots, no HadeethEnc bytes may enter the public Source Vault until archival retention is clarified. Edition/collection/numbering provenance remains a separate later gate. |

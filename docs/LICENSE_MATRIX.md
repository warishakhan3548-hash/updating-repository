...
| HadeethEnc Arabic | Hadith source candidate | **awaiting-licence** | Official HadeethEnc terms permit republication only under source-specific conditions and require downstream copies to update with newer source versions. Those terms do not clearly establish indefinite public redistribution of superseded immutable snapshots, so no bytes are mirrored. Edition/collection/numbering provenance remains a separate later gate. |

...
A dataset being redistributable today does not by itself prove that every superseded version may remain publicly mirrored forever. The project therefore treats historical-snapshot retention as a distinct Source Vault admission decision for every source that is ready for acquisition or preservation.

A source may be `awaiting-artifact`, contain preserved snapshot metadata, or be `production-approved` only when `historical_snapshot_retention_status` is `verified-allowed`. `unresolved` requires metadata-only `awaiting-licence`; `verified-not-allowed` requires metadata-only `rejected`. The runtime pack gate rechecks verified archival retention independently.

`latest_upstream_version_required` is a separate boolean. It controls whether an approved pack needs a source-bound signed freshness review; it does not grant archival rights. These mutable release requirements remain outside immutable acquisition provenance so legal/release review can evolve without rewriting preserved source history.

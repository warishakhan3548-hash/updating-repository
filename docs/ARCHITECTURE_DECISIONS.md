...
The v1 enrollment thresholds are experimental policy parameters, not permanent user-history semantics and not literature-derived optima. Scheduler-specific retention/deferral rules remain behind the adapter. Both may evolve after measured evaluation without rewriting append-only exposure/review history.

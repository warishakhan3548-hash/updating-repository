...
## Signing-hardening follow-up

| type | claim | source | confidence | product implication |
| --- | --- | --- | --- | --- |
| fact | TUF specification 1.0.36, last modified 2026-08-05, explicitly targets rollback, freeze, mix-and-match and key-compromise attacks and supports threshold/quorum trust. | https://theupdateframework.github.io/specification/latest/ | high | Keep a signed monotonic release ordering primitive now, while deferring full timestamp/snapshot/freshness machinery until downloadable updates exist. |
| fact | RFC 8785 requires JSON objects used for canonicalization not to contain duplicate property names and discusses interoperable numeric limits; it also preserves Unicode string data without normalization. | https://www.rfc-editor.org/rfc/rfc8785.html | high | Reject duplicate fields and unsafe cross-runtime numbers at the trust boundary. Our format is intentionally stricter (ASCII object keys, no floats) rather than claiming full JCS conformance. |
| finding | The workflow Action-pin regression test used over-escaped regular expressions, so the intended full-SHA policy was not being tested effectively. | repository audit | high | Correct the regex and keep a regression test; security policy that is not actually executed is documentation, not a gate. |
| finding | The first trusted-signature implementation authorized an approved Quran manifest before running the independent Quran semantic verifier. Both gates still failed closed, but the order blurred responsibilities. | repository audit | high | Run source/semantic fidelity before release authorization so a signature can never appear to bless altered sacred evidence. |
| design | `release_sequence` is required only for future `approved` manifests and is included in the signed payload; existing unsigned candidates remain byte-for-byte immutable. | project architecture | high | Future clients gain an unambiguous rollback-ordering value without rewriting historical content packs. |

...
## Anti-rollback trust hardening update

| type | claim | source | confidence | product implication |
| --- | --- | --- | --- | --- |
| fact | TUF treats rollback and indefinite-freeze attacks as distinct update threats; signature/integrity verification alone does not tell a client that an otherwise authentic artifact is older than one it has already trusted. | https://theupdateframework.io/docs/security/ | high | Authenticate monotonic release ordering now, but do not claim rollback/freshness protection until the Android client persists accepted ordering and freshness state. |
| fact | RFC 8032 specifies EdDSA including Ed25519 and provides implementation-oriented definitions and test vectors. | https://www.rfc-editor.org/rfc/rfc8032.html | high | Keep Ed25519 as the project signing algorithm while preserving a project-owned payload/trust schema so the crypto library remains replaceable. |
| finding | The existing project verifier authenticated manifest content but had no signed monotonic release ordering, no retired/revoked key windows, and ordinary JSON parsing accepted duplicate object keys using Python's last-value behavior. | repository audit at main 571ebf24b32494e49cfd0abe6100ef528340f517 | high | Harden the existing verifier rather than introducing a second signing subsystem: signed release sequence, bounded key lifecycle, domain separation, and strict duplicate-key parsing. |
| inference | A retired signing key needs a finite historical release-sequence ceiling; otherwise later compromise of that old private key could authenticate a future sequence while it remains present for historical verification. | TUF-inspired threat-model review | high | Bind each trusted key to explicit sequence validity windows and reject revoked/out-of-window signatures. |

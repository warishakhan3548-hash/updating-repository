...
## Release-authenticity update

| type | claim | source | confidence | product implication |
| --- | --- | --- | --- | --- |
| fact | TUF specification 1.0.36 was last modified 2026-08-05. It requires trusted root keys on clients, supports threshold signatures, recommends highly protected/offline root private keys, and explicitly targets rollback, freeze and mix-and-match attacks. | https://theupdateframework.github.io/specification/latest/ | high | Content-pack approval must verify signatures against an app/project-owned trust root; a future downloader must also retain trusted version state rather than accepting any historically valid signature. |
| fact | PyCA cryptography 50.0.1 was released 2026-08-25, and its documented Ed25519 API verifies a signature with a public key and raises `InvalidSignature` on failure. | https://pypi.org/project/cryptography/ and https://cryptography.io/en/latest/hazmat/primitives/asymmetric/ed25519/ | high | Use a pinned, replaceable implementation behind the pack-signature boundary instead of implementing Ed25519 primitives ourselves. |
| inference | A small TUF-inspired pack-signing boundary is sufficient for the current bundled-pack release gate; implementing the full online TUF role set before network content updates exist would add complexity without solving a present user problem. | TUF threat model + current repository architecture | high | Implement canonical signed manifests, versioned trust roots, threshold verification and signed release sequencing now; defer timestamp/snapshot network metadata until remote pack delivery is real. |
| finding | The repository can now cryptographically verify an `approved` pack, but no production release public key is provisioned. | repository audit/implementation | high | Keep `quran-core@1.0.4` candidate/unsigned; never generate a convenient private key in CI merely to obtain an approved label. |

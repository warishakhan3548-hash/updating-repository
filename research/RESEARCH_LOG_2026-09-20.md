...
## Release-ordering and parser-hardening update

| type | claim | source | confidence | product implication |
| --- | --- | --- | --- | --- |
| fact | TUF treats rollback resistance as a versioned trusted-metadata problem in addition to cryptographic signatures; a valid signature alone does not establish freshness. | https://theupdateframework.io/specification/latest/ | high | Put an explicit monotonic release-order field inside signed metadata now, and require persistent highest-seen state before automatic remote activation. |
| fact | RFC 8032 defines Ed25519 signatures over a byte string; protocol-specific domain separation is a standard way to keep signed bytes unambiguous across uses. | https://www.rfc-editor.org/rfc/rfc8032 | high | Prefix the project canonical manifest bytes with an Aaris content-pack domain separator and version the signing contract before any production key is enrolled. |
| fact | JSON permits implementations to differ in how duplicate object names and numeric ranges are handled; Python's default decoder otherwise keeps the last duplicate name. | https://www.rfc-editor.org/rfc/rfc8259 | high | Reject duplicate object keys before evidence/trust decisions and restrict signed integers to a cross-runtime-safe range. |
| finding | The first merged Ed25519 gate authenticated whole manifests and threshold keys but did not include explicit release ordering or per-key retirement/revocation windows. | repository audit at main 9869dbbf9696a40d682b105b7fc8cc8524bd5d7e | high | Evolve the existing verifier in place; do not add a second signing subsystem. |
| inference | A retired public key without a finite authorized release window can become dangerous if its old private key is compromised later. | TUF-style threat-model synthesis | high | Require finite `max_release_sequence` for retired keys and never accept revoked keys. |

...
| fact | QUL says resources are intended to be downloaded/packaged, while also identifying external resource origins. | https://qul.tarteel.ai/resources | high | Verify each resource's provenance/licence individually; no blanket approval. |\n| fact | QUL's current word-lemma/root resources use explicit word-location identifiers and local SQLite exports, and its integration guidance requires joining morphology to a word-by-word Quran script rather than reconstructing words from ayah whitespace. | https://qul.tarteel.ai/resources/morphology/75 and https://qul.tarteel.ai/resources/morphology/76 and https://qul.tarteel.ai/docs/tutorial-morphology-end-to-end | high | Keep Quran token identity as a verified source-backed layer; the reader may start at ayah level but must not fabricate word identities while licensing/provenance remains unresolved. |

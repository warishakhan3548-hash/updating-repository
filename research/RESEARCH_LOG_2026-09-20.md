...
| type | claim | source | confidence | product implication |
| --- | --- | --- | --- | --- |
| fact | Android's framework `AtomicFile` is available from API 17, commits file replacement only after a successful synced write, and explicitly provides no locking semantics. | https://developer.android.com/reference/android/util/AtomicFile | high | The minSdk-24 reader can persist one small monotonic trust record without a new storage dependency, but access must be serialized; use one process lock. |
| fact | Files under `Context.getNoBackupFilesDir()` are excluded from automatic backup/restore. | https://developer.android.com/reference/android/content/Context#getNoBackupFilesDir() | high | Keep highest-accepted content release state outside cloud backup so restoring an old backup cannot silently rewind it. |
| inference | Persisting the sequence only after the exact bundled bytes pass runtime hashing and activate prevents a corrupt/unusable candidate from advancing trust state merely because signed metadata was present. | repository threat-model review | high | Check sequence before replacement, verify bytes, activate, then atomically record the accepted maximum; fail closed on corrupt trust state. |
| limitation | App uninstall or clear-data removes app-private no-backup state. | Android app-private storage lifecycle | high | Describe this as bundled-release rollback resistance, not hardware-backed anti-tamper state or a complete remote-update protocol. |

...
| fact | SQLite FTS5 includes unicode61/trigram tokenizers and BM25 ranking. | https://sqlite.org/fts5.html | high | Strong boring baseline for measured multi-lane local retrieval. |\n| fact | SQLite documents a file-change counter in the database header and stores the `SQLITE_VERSION_NUMBER` of the library that most recently modified the database. | https://sqlite.org/fileformat.html | high | Treat a pack SHA-256 as identity for exact built bytes, but verify sacred source-to-runtime semantics independently instead of assuming byte identity alone proves evidence fidelity. |

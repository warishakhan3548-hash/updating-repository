...
| type | claim | source | confidence | product implication |
| --- | --- | --- | --- | --- |
| fact | TUF specification 1.0.36 (2026-08-05) treats threshold/quorum keys, revocation/rotation, rollback resistance and offline root trust as core update-security properties. | https://theupdateframework.github.io/specification/latest/ and https://theupdateframework.io/docs/security/ | high | Pack approval needs project-controlled trust keys and explicit rollback ordering; a hash or a single unbounded key is not a complete update-trust model. |
| fact | The Python `cryptography` Ed25519 API verifies a signature with the public key and raises `InvalidSignature` on failure. PyPI lists `cryptography 50.0.1` released 2026-08-25 and supports Python 3.13. | https://cryptography.io/en/latest/hazmat/primitives/asymmetric/ed25519/ and https://pypi.org/project/cryptography/ | high | Use a pinned, replaceable standard Ed25519 implementation rather than custom signature mathematics. |
| fact | QAC v0.4's official download terms permit verbatim redistribution/application use with attribution, while the official FAQ also says the data should not be used commercially and is for research/non-commercial use. | https://corpus.quran.com/download/ and https://corpus.quran.com/faq.jsp | high | Keep QAC out of production until the rights conflict is clarified; signing cannot make an uncleared source legally suitable. |
| design | The project signs a restricted deterministic JSON representation of the entire pack manifest except the signature container, includes a signed positive `release_sequence`, rejects floats, duplicate keys and invalid Unicode scalar values, and domain-separates the payload. | project architecture synthesis | high | Make signing bytes portable enough to implement identically on Android and eliminate parser ambiguity before a trust decision. |
| design | The production public-key registry starts empty with threshold 1. | project architecture synthesis | high | Shipping verifier capability cannot accidentally promote a candidate; a separate reviewed offline key ceremony is required. |

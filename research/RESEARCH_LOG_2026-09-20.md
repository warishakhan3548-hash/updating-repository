...
| fact | SinaLab/Birzeit lists QuranMorph under Classical Arabic as CC BY 4.0; the 2025 authors describe 77,429 manually lemmatized/POS-tagged words addressed by chapter:verse:word coordinates. The current official download form grants the free edition only to users with a recognized organization and professional official email, so this run did not obtain or mirror the artifact. The paper reports 6,235 verses, which must be reconciled against our 6,236-ayah Tanzil coordinate set from exact bytes before integration. | https://sina.birzeit.edu/resources/ and https://sina.birzeit.edu/quran/ and https://www.jarrar.info/publications/AHJ25.pdf | high | Treat QuranMorph as the clearest current morphology licensing lead, but keep it `awaiting-artifact`; obtain the exact artifact through an authorized publisher path, preserve its licence/provenance/checksum, then run full coordinate/text alignment before production use. |
...
| type | claim | source | confidence | product implication |
| --- | --- | --- | --- | --- |
| fact | TUF uses trusted roles, key lists and signature thresholds, and its security guidance includes recovery from compromised keys rather than trusting one key forever. | https://theupdateframework.io/docs/metadata/ and https://theupdateframework.io/docs/faq/ | high | Keep threshold signing and make key rotation/revocation an explicit project-controlled trust operation. |
| finding | Main already authenticates a monotonic `release_sequence` and domain-separates the signing payload, but its public-key policy does not distinguish active, retired and revoked keys or constrain an old key to a historical sequence range. | repository audit at main 615026b224aa74547abd50e721292fc90bcef6bd | high | Add lifecycle state and sequence windows to the existing trust root rather than creating a parallel signing system. |
...
## Android reader quality-gate update
...
| fact | Android `AtomicFile` provides fail-safe replacement semantics but explicitly provides no file-locking semantics; callers must ensure mutual exclusion around concurrent access. | https://developer.android.com/reference/android/util/AtomicFile | high | Keep atomic disk replacement, and serialize the full pack activation transaction with one process-wide monitor in the current single-process app. |
...
## Offline release-key ceremony tooling

| type | claim | source | confidence | product implication |
| --- | --- | --- | --- | --- |
| fact | OpenSSL `genpkey` supports Ed25519 key generation and encrypted PEM private-key output when an encryption cipher is selected. | https://docs.openssl.org/master/man1/openssl-genpkey/ | high | The production release key can be generated on a deliberately offline machine as encrypted PKCS#8 PEM without inventing a project-specific secret-key file format. |
| fact | PyCA Cryptography loads encrypted PEM private keys with an explicit password and its Ed25519 API signs arbitrary bytes, verifies signatures and exposes raw public-key bytes. | https://cryptography.io/en/latest/hazmat/primitives/asymmetric/ed25519/ and https://cryptography.io/en/latest/hazmat/primitives/asymmetric/serialization/ | high | Reuse the pinned release-verification dependency behind a project-owned signing contract; keep the library replaceable. |
| inference | A signing command that also changes `candidate` to `approved` would collapse independent review and authorization into one privileged operation. | release-process threat-model review | high | Require `review_status=approved` before signing; the signer authorizes reviewed bytes but does not make the review decision. |
| decision | Production private keys are neither generated nor stored by repository automation. The signer requires encrypted key material outside the repository, prompts for its password, self-verifies each signature, and creates a new signed output instead of overwriting inputs. | repository architecture decision | high | Keep GitHub/CI outside the secret-key trust boundary while making the ceremony deterministic and testable. |

...
| QuranEnc states translation content may be downloaded/re-published if it is not modified, QuranEnc/publisher is clearly referenced, the version is stated, transcript information is retained, notes are reported, newer source versions are tracked, and inappropriate advertising is avoided. | https://quranenc.com/en/home | 2026-09-20 | High for the published general terms; Medium for exact resource-scope interpretation | Treat the source as `awaiting-artifact`, not production-approved. Preserve the exact applicable terms with the data snapshot before promotion. | Do not transform or paraphrase the archived source content; derived learning aids must remain separate from the preserved assertion. |
...
1. obtain the authoritative Arabic translation-list metadata and confirm `arabic_seraj` at version 1.0.0;

...
| hypothesis | A versioned v1 interruption rule can use scheduler retrievability >= 0.60 as a conservative substitution floor while leaving this value replaceable and unevaluated. | Product hypothesis, not a literature-derived optimum | 2026-09-21 | Experimental | Keep the threshold explicit in policy config and make no retention-quality claim until measured. |
...
- An already-due scheduler review with unknown or low retrievability fails closed to explicit review.
- A suitable predicted natural encounter may replace an interruptive review only when the versioned substitution rule allows it.

...
Export uses SQLite's backup API. Backup format v1 accepts only user schema v2; a legacy pre-v2 database must run the existing tested migration first. Import validates archive structure, hash, database integrity, supported schema version and required durable tables before a destination is published. Restore refuses to overwrite an existing database so Android integration cannot accidentally become an uncontrolled live-database replacement path.

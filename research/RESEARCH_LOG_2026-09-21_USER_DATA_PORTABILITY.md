...
Export uses SQLite's backup API. Import validates archive structure, hash, database integrity, supported schema version and required durable tables before a destination is published. Restore refuses to overwrite an existing database so Android integration cannot accidentally become an uncontrolled live-database replacement path.

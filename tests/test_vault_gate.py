...
class VaultGateTests(unittest.TestCase):
    ...
    def test_unregistered_file_under_source_vault_fails_closed(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            source, artifact, _ = self._valid_snapshot(root)
            path = self._registry(root, source)
            (artifact.parent / "undeclared.bin").write_bytes(b"not registered")

            with self.assertRaisesRegex(
                VaultGateError, "unregistered file under source-vault"
            ):
                validate_registry(path)

    def test_metadata_only_blocked_source_cannot_hide_vault_bytes(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            hidden = root / "source-vault" / "hadith" / "blocked" / "raw.json"
            hidden.parent.mkdir(parents=True)
            hidden.write_bytes(b"unreviewed evidence")
            path = self._registry(
                root,
                {
                    "source_id": "blocked",
    ...
    def test_provenance_checksum_file_is_accounted_and_verified(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            source, artifact, provenance_path = self._valid_snapshot(root)
            checksum = artifact.parent / "sha256.txt"
            checksum.write_text(
                f"{source['sha256']}  {artifact.name}\n",
                encoding="utf-8",
            )
            provenance = json.loads(provenance_path.read_text(encoding="utf-8"))
            provenance["checksum_file"] = checksum.relative_to(root).as_posix()
            provenance_path.write_text(json.dumps(provenance), encoding="utf-8")
            source["provenance_sha256"] = hashlib.sha256(
                provenance_path.read_bytes()
            ).hexdigest()

            validate_registry(self._registry(root, source))

    def test_provenance_checksum_file_mismatch_fails_closed(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            source, artifact, provenance_path = self._valid_snapshot(root)
            checksum = artifact.parent / "sha256.txt"
            ...
            with self.assertRaisesRegex(
                VaultGateError,
                "provenance checksum_file does not match the pinned artifact",
            ):
                validate_registry(self._registry(root, source))

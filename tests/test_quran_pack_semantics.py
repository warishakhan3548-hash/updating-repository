...
PACK_REL = Path("content-packs/quran-core/1.0.0")
...
class QuranPackSemanticTests(unittest.TestCase):
    ...
    def _copy_release_fixture(self, destination: Path) -> tuple[Path, Path]:
        ...
        return destination / "source-vault" / "registry.json", destination / MANIFEST_REL
    ...
    def test_pack_gate_rejects_semantic_quran_tamper_even_with_fresh_file_hash(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # Simulate an attacker or broken release process that also refreshes the
            # outer artifact hash. The generic byte-integrity check now passes, so
            # only source-to-pack semantic verification can reject this artifact.

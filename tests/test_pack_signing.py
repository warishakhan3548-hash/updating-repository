...
class PackSigningTests(unittest.TestCase):
    ...
    def test_non_ascii_object_keys_are_rejected(self) -> None:
        manifest = self._manifest()
        manifest["clé"] = "value"
        with self.assertRaisesRegex(PackSignatureError, "object keys must be ASCII"):
            signature_payload(manifest)

    def test_invalid_unicode_surrogates_are_rejected(self) -> None:
        manifest = self._manifest()
        manifest["source_attribution"] = "\ud800"
        with self.assertRaisesRegex(PackSignatureError, "Unicode scalar values"):
            signature_payload(manifest)

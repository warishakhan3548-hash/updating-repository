...
def authorize_capture(
    ...
    *,
    ...
    historical_retention_status="verified-allowed",
):
                ...
                "sources": [
                    {
                        ...
                        "release_requirements": {
                            ...
                            "historical_snapshot_retention_status": historical_retention_status,
                        },
                        "vault_artifact": None,
...
class CaptureTests(unittest.TestCase):
    ...
    def test_unresolved_archival_retention_blocks_before_network(
        self,
    ):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            authorize_capture(root, historical_retention_status="unresolved")

            def should_not_fetch(url):
                self.fail(
                    "network fetch attempted before archival-retention authorization"
                )

            with self.assertRaisesRegex(
                CaptureError, "historical snapshot retention permission"
            ):

...
class CaptureTests(unittest.TestCase):
    ...
    def test_unresolved_archival_retention_blocks_before_network(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            authorize_capture(
                root,
                historical_retention_status="unresolved",
            )

            def should_not_fetch(url):
                self.fail(
                    "network fetch attempted before archival retention clearance"
                )

            with self.assertRaisesRegex(
                CaptureError, "historical snapshot retention"
            ):
                capture_snapshot(
                    root,

...
This module deliberately does not trust the importer merely because the resulting
SQLite file has a matching release hash. It reconstructs expected Quran rows
from the production-approved Source Vault artifact and compares the runtime pack
row-by-row, including derived search lanes and source assertions.
"""
...
PACK_ID = "quran-core"
SUPPORTED_CONTENT_VERSION = "1.0.0"
...
def _verify_no_unexpected_evidence(connection: sqlite3.Connection) -> None:
    ...
    if non_empty:
        raise QuranPackSemanticError(
            f"quran-core 1.0.0 contains unexpected evidence rows: {non_empty}"
        )
...
def verify_quran_core_pack(
    ...
    if manifest.get("content_version") != SUPPORTED_CONTENT_VERSION:
        raise QuranPackSemanticError(
            "unsupported quran-core content_version for semantic verification: "
            f"{manifest.get('content_version')!r}"
        )
    if manifest.get("source_id") != SOURCE_ID:
        raise QuranPackSemanticError("quran-core source_id mismatch")
    if manifest.get("record_count") != 6236:
        raise QuranPackSemanticError("quran-core manifest record_count must be 6236")
    if manifest.get("dependencies") != []:
        raise QuranPackSemanticError(
            "quran-core 1.0.0 must not claim external pack dependencies"
        )

    source, _, expected_rows = load_production_source(root)
    artifact = artifact or _artifact_from_manifest(root, manifest)
...
def main() -> None:
    if len(sys.argv) not in {1, 2}:
        raise SystemExit(
            ...
            "[content-packs/quran-core/1.0.0/manifest.json]"
        )
    root = Path(__file__).resolve().parents[1]
    manifest_path = (
        Path(sys.argv[1])
        if len(sys.argv) == 2
        else root / "content-packs" / "quran-core" / "1.0.0" / "manifest.json"
    )

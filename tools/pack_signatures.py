...
def _validate_active_release_policy(
    keyring: dict[str, Any],
) -> tuple[dict[str, Any], set[str], int]:
    if keyring.get("state") != "active":
    ...
    for key_id in key_ids:
        ...
        if not isinstance(key, dict):
            raise PackSignatureError(f"missing trusted key {key_id}")
        if key.get("algorithm") != "ed25519":
            raise PackSignatureError(
                f"unsupported trusted-key algorithm for {key_id}"
            )
        public_key = key.get("public_key")
        computed_id = key_id_for_ed25519_public_key(public_key)
...
def verify_approved_manifest(
    ...
    keys, authorized, threshold = _validate_active_release_policy(keyring)

...
def _validate_download(
    ...
    *,
    ...
    if download.requested_url != expected_url:
        raise CaptureError(
            f"{label}: requested URL mismatch: {download.requested_url!r}"
        )
    _require_quranenc_https(download.requested_url, f"{label} requested URL")
    _require_quranenc_https(download.final_url, f"{label} final URL")
    if download.status != 200:
        raise CaptureError(f"{label}: HTTP status is not 200")
    if not download.body:
        raise CaptureError(f"{label}: empty response")
    if len(download.body) > MAX_RESPONSE_BYTES:
        raise CaptureError(
...
def validate_sura_response(raw: bytes, expected_sura: int) -> list[int]:
    ...
    expected = list(
        range(1, EXPECTED_AYAH_COUNTS[expected_sura - 1] + 1)
    )
    if ayahs != expected:
        raise CaptureError(
            f"Surah {expected_sura}: coordinate mismatch; "
            f"expected {len(expected)} ordered ayahs, found {len(ayahs)}"
        )
    return ayahs
...
def capture(
    ...
    *,
    ...
    try:
        ...
        postflight_index = fetcher(SOURCE_INDEX_URL)
        _validate_download(
            postflight_index,
            expected_url=SOURCE_INDEX_URL,
            label="source index postflight",
        )
        if postflight_index.body != source_index.body:
            raise CaptureError(
                "QuranEnc source index changed during capture; discard and retry"
            )
...
def _expected_capture_paths() -> set[str]:
    return {
        "SOURCE_INDEX.html",
        "LICENSE_SOURCE.html",
        "SOURCE_PAGE.html",
...
def _expected_url_for_path(relative: str) -> str:
    if relative == "SOURCE_INDEX.html":
        return SOURCE_INDEX_URL
    ...
    if relative.startswith(prefix) and relative.endswith(".json"):
        stem = relative[len(prefix):-5]
        if len(stem) == 3 and stem.isdigit():
            sura = int(stem)
            if 1 <= sura <= 114:
                return SURA_URL_TEMPLATE.format(sura=sura)
    raise CaptureError(f"unexpected capture file path: {relative!r}")


def _validate_retrieved_at(value: object) -> None:
    if not isinstance(value, str) or not value.endswith("Z"):
        raise CaptureError("retrieved_at must be an explicit UTC timestamp")
    try:
        parsed = datetime.fromisoformat(value[:-1] + "+00:00")
    except ValueError as exc:
        raise CaptureError("retrieved_at is not valid ISO-8601") from exc
    if parsed.utcoffset() != timezone.utc.utcoffset(parsed):
        raise CaptureError("retrieved_at must be UTC")
...
def validate_existing(output: Path) -> dict:
    ...
    _validate_retrieved_at(provenance.get("retrieved_at"))
    ...
    for record in records:
        ...
        if relative not in expected_paths:
            raise CaptureError(f"unexpected capture file record: {relative!r}")
        ...
        expected_url = _expected_url_for_path(relative)
        if record.get("requested_url") != expected_url:
            raise CaptureError(f"{relative}: requested URL mismatch")
    ...
    if seen != expected_paths:
        ...
        raise CaptureError(
            f"capture file set mismatch; missing={missing}, extra={extra}"
        )
...
def capture_snapshot(
    ...
    *,
    ...
    destination = repo_root.resolve() / DEFAULT_OUTPUT
    capture(destination, fetcher, retrieved_at=now)
    return destination
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Capture or offline-verify QuranEnc arabic_seraj v1.0.0 "
            "Source Vault bytes"
        )
    )
    parser.add_argument(
    ...
    try:
        ...
        print(f"QuranEnc Source Vault operation FAILED: {exc}")
        return 1

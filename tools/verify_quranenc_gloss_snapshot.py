...
This verifier intentionally performs no network access. It checks the immutable
review-only capture independently of the acquisition path before later review or
registry promotion.
"""
...
def _verify_requests(
    ...
    external_files = {
        "LICENSE_SOURCE.html": licence_bytes,
        "SOURCE_PAGE.html": source_page_bytes,
    }
    expected_names = set(tar_files) | set(external_files)
    ...
    for record in requests:
        ...
        body = external_files.get(stored_as, tar_files.get(stored_as))
        if body is None:
            raise CaptureError(f"{stored_as}: captured body missing")
...
def _verify_bound_artifact(
    ...
    *,
    ...
    if not isinstance(metadata, dict):
        raise CaptureError(f"{field} metadata is incomplete")
    if metadata.get("path") != expected_path:
        raise CaptureError(f"{field} path mismatch")
    if metadata.get("byte_size") != len(body):
        raise CaptureError(f"{field} byte size mismatch")
    if metadata.get("sha256") != sha256_bytes(body):
        raise CaptureError(f"{field} SHA-256 mismatch")
...
def verify_snapshot(repo_root: Path) -> dict:
    ...
    source_page_bytes = _require_file(root, "SOURCE_PAGE.html")
    ...
    _verify_bound_artifact(
        manifest,
        field="raw_snapshot",
        expected_path="raw-snapshot.tar",
        body=raw_snapshot,
    )
    _verify_bound_artifact(
        manifest,
        field="licence_snapshot",
        expected_path="LICENSE_SOURCE.html",
        body=licence_bytes,
    )
    _verify_bound_artifact(
        manifest,
        field="source_page_snapshot",
        expected_path="SOURCE_PAGE.html",
        body=source_page_bytes,
    )
    ...
    expected_source_page = (
        VAULT_RELATIVE / "SOURCE_PAGE.html"
    ).as_posix()
    expected_manifest = (
    ...
    fixed_provenance = {
        ...
        "source_page_snapshot": expected_source_page,
    ...
    expected_checksums = {
        ...
        "SOURCE_PAGE.html": sha256_bytes(source_page_bytes),

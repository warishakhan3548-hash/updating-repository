...
def validate_checksum_set(artifact: Path, source_id: str) -> int:
    """Verify a project-controlled checksum ledger and every member it binds."""
    try:
    ...
    for line_number, line in enumerate(lines, start=1):
        ...
        member = artifact.parent / Path(*posix_rel.parts)
...
def validate_registry(registry_path: Path) -> None:
    ...
    for source in sources:
        ...
        if artifact_kind == "sha256-set":
            validate_checksum_set(artifact, source_id)

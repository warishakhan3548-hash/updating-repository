...
def validate_registry(registry_path: Path) -> None:
    ...
    for source in sources:
        ...
        if status == "rejected" and snapshot_fields_present:
            raise VaultGateError(
                f"{source_id}: rejected source must not preserve "
                "project-controlled snapshot bytes"
            )
        ...
        if retention_clearance_required and retention_status != "verified-allowed":
            raise VaultGateError(

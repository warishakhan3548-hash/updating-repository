...
def main():
    ...
    report = {
        ...
        'quran_audio_catalog_sha256': digest(audio_catalog),
        'quran_audio_source_revision': catalog['source_revision'],
        'quran_audio_style': catalog.get('style'),
        'quran_audio_words': int(catalog['canonical_quran_audio_words']),
        'quran_audio_surah_packs': int(catalog['surahs']),
        'quran_audio_optional_download_bytes': int(catalog['total_bytes']),
        ...
        'legacy_bundled_quran_audio_words': int(audio_manifest['word_count']) if audio_manifest is not None else 0,

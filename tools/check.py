...
def main():
    ...
    clock_seed='if(lastEventTime==0)try(Cursor c=getReadableDatabase().rawQuery("SELECT COALESCE(MAX(at),0) FROM event",null)){if(c.moveToFirst())lastEventTime=c.getLong(0);}'
    clock_guard='if(lastEventTime>now+MAX_FUTURE_EVENT_SKEW)lastEventTime=now;'
    clock_values='ContentValues v=new ContentValues();'
    assert 'MAX_FUTURE_EVENT_SKEW=5*Recall.MINUTE' in learning_store_text and clock_seed in learning_store_text and clock_guard in learning_store_text, 'Learning event creation must bound an implausibly future clock'
    assert learning_store_text.index(clock_seed)<learning_store_text.index(clock_guard)<learning_store_text.index(clock_values), 'Learning clock recovery must run on every event, including after an in-process wall-clock rollback'

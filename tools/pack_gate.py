...
def validate_manifest(manifest_path: Path, registry_path: Path) -> None:
    ...
    if manifest["schema_version"] == 2:
        ...
        notice = _safe_repo_file(
            root, manifest["notice_path"], "notice_path", "content-packs"
        )
        notice_hash = _lower_sha256(manifest["notice_sha256"], "notice_sha256")
        if sha256_file(notice) != notice_hash:
            raise PackGateError(f"{manifest_path}: notice_sha256 mismatch")
        try:
            notice_text = notice.read_text(encoding="utf-8")
        except UnicodeDecodeError as exc:
            raise PackGateError(f"{manifest_path}: notice must be UTF-8 text") from exc
        if not notice_text.strip():
            raise PackGateError(f"{manifest_path}: notice must not be empty")

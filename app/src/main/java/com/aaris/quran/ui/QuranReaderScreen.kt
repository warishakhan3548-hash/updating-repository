...
import com.aaris.quran.data.AndroidReaderAyah
...
private sealed interface ReaderState {
    ...
    data class Ready(
        val ayahs: List<AndroidReaderAyah>,
...
fun QuranReaderScreen(
    repository: QuranReaderRepository,
    onOpenSource: () -> Unit,
) {
    var surah by rememberSaveable { mutableIntStateOf(SurahNavigation.FIRST_SURAH) }
    ...
    val state by produceState<ReaderState>(
        ...
        value = try {
            ReaderState.Ready(
                ayahs = repository.loadSurah(surah),
                packInfo = repository.packInfo(),
            )
        } catch (_: Throwable) {
            ReaderState.Error
        }
    ...
    Scaffold(
        ...
        Column(
            ...
            SurahBar(
                surah = surah,
                onPrevious = { surah = SurahNavigation.previous(surah) },
                onNext = { surah = SurahNavigation.next(surah) },
            )
...
private fun SurahBar(
    ...
    Row(
        ...
        OutlinedButton(
            onClick = onPrevious,
            enabled = surah > SurahNavigation.FIRST_SURAH,
        ) {
        ...
        OutlinedButton(
            onClick = onNext,
            enabled = surah < SurahNavigation.LAST_SURAH,
        ) {
...
@Composable
private fun AyahRow(ayah: AndroidReaderAyah) {
    Column(
        modifier = Modifier
            .fillMaxWidth()

...
public final class AmbientRecallService extends Service {
    ...
    @Override public void onCreate(){
        super.onCreate();app=(QuranApp)getApplication();windows=getSystemService(WindowManager.class);
    ...
    private void showCard(String shownTarget){
        ...
        LinearLayout shell=column(this);shell.setBackground(new Surface(this,Surface.Kind.SHEET,true));pad(shell,18,14);
        LinearLayout bar=row(this);LinearLayout title=column(this);
        TextView brand=text(this,"AARIS · YAAD KA LAMHA",10,GOLD);brand.setLetterSpacing(.1f);title.addView(brand);
        title.addView(text(this,app.content.surah(ayah.surah).name+" · "+ayah.surah+":"+ayah.number,13,MUTED));bar.addView(title,new LinearLayout.LayoutParams(0,-2,1));
        TextView close=control("×",this::dismissCard);close.setContentDescription("Card band karein; baad mein phir yaad dilayein");close.setTextSize(27);bar.addView(close,new LinearLayout.LayoutParams(dp(this,48),dp(this,48)));shell.addView(bar);
        int height=displayHeight();int maxBody=Math.max(dp(this,80),Math.min(dp(this,410),height-dp(this,220)));
        ScrollView scroll=new ScrollView(this){@Override protected void onMeasure(int w,int h){super.onMeasure(w,View.MeasureSpec.makeMeasureSpec(maxBody,View.MeasureSpec.AT_MOST));}};
        scroll.setFillViewport(false);scroll.setOverScrollMode(View.OVER_SCROLL_NEVER);LinearLayout body=column(this);pad(body,0,12);scroll.addView(body);shell.addView(scroll,new LinearLayout.LayoutParams(-1,-2));
        ...
        TextView hint=text(this,instruction,14,MUTED);hint.setGravity(Gravity.CENTER);pad(hint,0,12);body.addView(hint);
        LinearLayout answer=column(this);answer.setVisibility(View.GONE);body.addView(answer);
        ...
        reveal.setOnClickListener(v->{
            ...
            if(word!=null){TextView meaning=text(this,word.gloss(app.learning.get("language","hi")),24,INK);meaning.setGravity(Gravity.CENTER);pad(meaning,0,12);answer.addView(meaning);}
            else {if(edge!=null){TextView ref=text(this,edge.to.surah+":"+edge.to.number+" · Agli ayah ka aaghaz",12,GOLD);answer.addView(ref);}answer.addView(arabic(original,28));}
            TextView question=text(this,"Dekhne se pehle kitna yaad tha?",12,MUTED);question.setGravity(Gravity.CENTER);pad(question,0,14);answer.addView(question);
            ...
            for(int row=0;row<2;row++){LinearLayout ratings=Glass.row(this);
                ...
                });LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(0,-2,1);p.setMargins(col==0?0:dp(this,6),0,0,dp(this,6));ratings.addView(rating,p);}answer.addView(ratings);
            }
        });
        TextView stop=text(this,"Session band karein",12,MUTED);stop.setGravity(Gravity.CENTER);stop.setMinimumHeight(dp(this,44));stop.setFocusable(true);stop.setOnClickListener(v->finish("Session band hai"));shell.addView(stop,new LinearLayout.LayoutParams(-1,-2));
        int width=Math.min(dp(this,410),displayWidth()-dp(this,32));
        ...
        params.gravity=Gravity.TOP|Gravity.CENTER_HORIZONTAL;params.y=dp(this,44);params.setTitle("Aaris Quran recall");
        // The window covers only its card. Touches outside it belong to the underlying app.
        windows.addView(shell,params);card=shell;
    }
    private int displayWidth(){return Build.VERSION.SDK_INT>=30?windows.getCurrentWindowMetrics().getBounds().width():getResources().getDisplayMetrics().widthPixels;}
    private int displayHeight(){return Build.VERSION.SDK_INT>=30?windows.getCurrentWindowMetrics().getBounds().height():getResources().getDisplayMetrics().heightPixels;}
    ...
    private TextView arabic(String value,int size){TextView t=text(this,value,size,INK);t.setTypeface(font);t.setTextDirection(View.TEXT_DIRECTION_RTL);t.setGravity(Gravity.CENTER);t.setLineSpacing(dp(this,8),1.05f);return t;}
    private TextView control(String value,Runnable action){TextView t=text(this,value,14,INK);t.setGravity(Gravity.CENTER);pad(t,12,10);t.setMinimumHeight(dp(this,48));t.setBackground(Glass.touch(this,Surface.Kind.BUTTON,true));t.setFocusable(true);t.setOnClickListener(v->action.run());return t;}

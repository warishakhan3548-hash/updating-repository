...
public final class RecitationService extends Service {
    ...
    private void play(){
        int token=++generation;if(pending!=null)pending.cancel(true);releasePlayer();paused=false;buffering=true;app.recitationActive=true;
    ...
    private void pause(){
        if(player!=null)try{if(player.isPlaying()){player.pause();buffering=false;paused=true;update("Paused · "+label());return;}}catch(IllegalStateException ignored){}
        generation++;if(pending!=null)pending.cancel(true);releasePlayer();buffering=false;paused=true;update("Paused · "+label());
    }
    private void resume(){if(player!=null)try{if(audio.requestAudioFocus(focus)==AudioManager.AUDIOFOCUS_REQUEST_GRANTED){buffering=false;player.start();paused=false;update(label());return;}}catch(IllegalStateException ignored){}play();}
    private void update(String message){app.recitationSurah=surah;app.recitationAyah=ayah;app.recitationLabel=message;app.main.post(()->{if(app.recitationChanged!=null)app.recitationChanged.run();});session.setMetadata(new MediaMetadata.Builder().putString(MediaMetadata.METADATA_KEY_TITLE,message).build());
        int state=buffering?PlaybackState.STATE_BUFFERING:paused?PlaybackState.STATE_PAUSED:PlaybackState.STATE_PLAYING;

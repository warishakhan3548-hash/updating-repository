...
public final class RecitationService extends Service {
    ...
    @Override public void onCreate(){super.onCreate();app=(QuranApp)getApplication();audio=getSystemService(AudioManager.class);
        ...
        focus=new AudioFocusRequest.Builder(AudioManager.AUDIOFOCUS_GAIN).setAudioAttributes(attrs).setOnAudioFocusChangeListener(change->{
            if(change==AudioManager.AUDIOFOCUS_GAIN){
                if(resumeAfterTransientFocusLoss&&paused){resumeAfterTransientFocusLoss=false;resume();}
                return;
            }
            if(paused){resumeAfterTransientFocusLoss=false;return;}
    ...
    private void resume(){
        ...
        if(player!=null)try{
            if(audio.requestAudioFocus(focus)!=AudioManager.AUDIOFOCUS_REQUEST_GRANTED){buffering=false;paused=true;update("Paused · "+label());return;}
            buffering=false;player.start();paused=false;update(label());return;

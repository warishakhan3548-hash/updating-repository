...
void main() {
  ...
  test('decimal strengths remain distinct throughout inventory identity', () {
    final records = [
      stock('a', strength: '2.5mg'),
      stock('b', strength: '25mg'),
    ];
  ...
  test(
    'candidate limit keeps nearest equal-name stock regardless of input order',
    () {
      final records = List.generate(
        ...
        (i) => stock(
          'stock_$i',
          expiry: dateText(
            civilDay(contractToday).add(Duration(days: 360 - i)),
          ),
        ),
      );
  ...
  test(
    'only expired inventory creates reorder without silently marking SOLD',
    () {
      final record = stock('expired', quantity: 100, expiry: '2026-09-06');
      final stats = TrackingStats(
        medicines: [record],
        sales: const [],
        range: TrackingRange.lastDays(DateTime(2026, 8, 1), 7),
        today: contractToday,
      );
  ...
  test(
    'a stale editor cannot archive or record a sale after another edit',
    () async {
      ...
      await controller.save(
        controller.records.single.patch({'quantity': 20}),
        expectedRevision: reviewed,
      );
      await expectLater(
        controller.archive('a', 'Correction', expectedRevision: reviewed),
        throwsStateError,
      );
      await expectLater(
        controller.recordSale('a', quantity: 1, expectedRevision: reviewed),
        throwsStateError,
      );
      await expectLater(
        controller.archiveAll(expectedRevision: reviewed),
        throwsStateError,
      );
  ...
  test(
    'AI selected actions are frozen before asynchronous preparation',
    () async {
      final controller = PharmacyController(
        MemoryInventoryStorage(),
        clock: () => contractToday,

...
void main() {
  group('quick intake field-label boundaries', () {
    test(
      'field-label prefixes inside medicine names remain medicine evidence',
      () {
        ...
        final barcodeName = parseMedicineAddPrefill(
          'add medicine BarcodeX 20',
        )!;
    ...
    test(
      'labeled fact words cannot be captured from medicine-name suffixes',
      () {
    ...
    test(
      'Devanagari price digits are normalized only when explicitly labeled',
      () {
        final draft = parseMedicineAddPrefill(

...
final class NativeLlamaRuntime {
  ...
  void _resetPromptContext(_LoadedModel loaded) {
    // Every command supplies a complete, independent prompt. Keep model weights
    // but discard prior sequence/KV state so medicine A cannot leak into B.
    final memory = _bindings.llama_get_memory(loaded.context);
    if (memory != nullptr) _bindings.llama_memory_clear(memory, true);
  }

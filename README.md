...
Phase 0A–0C is operational, the first Quran evidence source has passed the production Source Vault gate, and Phase 1 has a minimal offline Android reader. Canonical Quran schema v3 and trusted Ed25519 release verification are now implemented; production approval remains blocked until offline release-key custody is bootstrapped.
...
This is not yet a finished reader application. The reader deliberately withholds word meaning/morphology because no word-level linguistic source has passed the legal Source Vault gate. Learning, Hadith retrieval and external-AI evidence workflows remain later phases.
...
The current `quran-core 1.1.0` candidate uses manifest schema v3 and is bound to the deterministic canonical artifact `canonical/quran-core/1.0.0/ayahs.jsonl`. It contains 6,236 ayahs, preserves source display Arabic separately from search lanes, and remains unsigned/candidate until release review and signing. The older 1.0.4 candidate remains immutable for reproducibility.

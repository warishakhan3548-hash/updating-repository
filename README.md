...
- Independent source durability is also fail-closed at final approval: `policy/source_backups.json` must bind the exact Source Vault SHA-256 to a verified independent backup before a signed `approved` pack can pass. Current Tanzil backup status is honestly tracked as pending rather than assumed.

...
Phase 0A–0C is operational and Phase 1 has a minimal offline Android reader on the trusted read-only Reader Core boundary. The deterministic canonical Quran layer is now the bridge from the preserved Source Vault snapshot into replaceable runtime packs.

**Production-approved source today:** Tanzil Quran Text v1.1, exact pinned Uthmani `txt-2` snapshot.

**Current Quran runtime candidate:** `quran-core 1.1.0` (manifest schema v3), generated from the project-controlled Tanzil snapshot through `canonical/quran-core/1.0.0/ayahs.jsonl`. It contains 6,236 ayahs and remains `candidate` / unsigned.
...
- Canonical Quran JSONL is the long-lived semantic reproducibility anchor between Source Vault and SQLite runtime bytes.
...
- Release approval is fail-closed: approved manifests require signed release ordering and must pass the authoritative trusted-key pack gate.
...
`tools/build_quran_canonical.py` deterministically creates `canonical/quran-core/1.0.0/ayahs.jsonl`, whose SHA-256 is the durable semantic anchor. `tools/build_quran_core.py` builds `content-packs/quran-core/1.1.0/content.sqlite` from that canonical layer using manifest schema v3. The Android debug reader bundles this 1.1.0 candidate directly.

The pack remains unsigned/candidate. Release builds stay blocked until offline release-key custody is deliberately bootstrapped, the public trust material is reviewed, and an immutable approved pack carries a signed `release_sequence`. No private release key belongs in GitHub.

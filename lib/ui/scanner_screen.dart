...
class _ScannerScreenState extends State<ScannerScreen>
  ...
  Future<void> _stopCamera() async {
    ...
    // Detach synchronously, then give every operation owned by this retired
    // camera session one shared grace period. Waiting sequentially multiplied
    // lifecycle latency, while leaving the futures attached made a later resume
    // wait for the same native callbacks again. The recognizer's own busy lease
    // still blocks overlapping OCR until late work really settles.
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    // Only the first background transition tears the session down; resume enters
    // the same serialized restart owner instead of queuing a second stop first.
